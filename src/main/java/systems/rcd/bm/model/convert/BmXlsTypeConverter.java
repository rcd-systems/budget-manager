...
public class BmXlsTypeConverter
{
    ...
    {
        final String typeName = xlsRow.get( 0 ).getStringValue();
        final Type type = getType( typeName );

        xlsRow.stream().skip( 1 ).filter( Objects::nonNull ).map( cell -> getType( cell.getStringValue() ) ).forEach( type::addSubType );
