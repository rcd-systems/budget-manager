...
public class BmXlsTransferConverter
    implements BmModelConstants
{
    ...
    {
        return xlsSheet.stream().skip( 1 ).map( this::convert ).filter( Objects::nonNull ).collect( Collectors.toList() );
    }
    ...
    private Transfer convert( final RcdXlsRow xlsRow )
    {
        ...
        transfer.setComments( comments ).setSourceAccount( srcAccount ).setSourceDate( toLocalDate( srcDateInstant ) ).setTargetAccount(
            tgtAccount ).setTargetDate( toLocalDate( tgtDateInstant ) );
