...
public class BmModelService
    implements RcdService, BmModelConstants
{
    ...
    private RcdXlsWorkbook parseInputFile()
        throws Exception
    {
        ...
        throws BmException
    {
        ...
        final BmXlsTransferConverter transferConverter = new BmXlsTransferConverter().accountMap( accountMap ).typeMap( typeMap );
    ...
    private void index()
    {
        rootAccounts = accountMap.values().stream().filter( account -> account.getParent() == null ).collect( Collectors.toList() );

        rootTypes = typeMap.values().stream().filter( type -> type.getParent() == null ).collect( Collectors.toList() );
    ...
    {
        return transfersByDate.findTransfers( year, month ).collect( Collectors.toList() );
    }

    public List<Transfer> findIncomingTransfers( final Integer year, final Integer month )
    {
        return transfersByTargetDate.findTransfers( year, month ).collect( Collectors.toList() );
    }

    public List<Transfer> findOutgoingTransfers( final Integer year, final Integer month )
    {
        return transfersBySourceDate.findTransfers( year, month ).collect( Collectors.toList() );
    }

    public Set<Integer> findYears()
    {
        return transfersByDate.findYears().collect( Collectors.toSet() );
    }
    ...
    public double findInitialAmount( final Integer year, final Integer month, final String account )
    {
        ...
        initialAmount -=
            transfersBySourceDate.findTransfersBefore( year, month ).filter( transfer -> transfer.isOutgoing( account ) ).mapToDouble(
                Transfer::getAmount ).sum();

        initialAmount +=
            transfersByTargetDate.findTransfersBefore( year, month ).filter( transfer -> transfer.isIncoming( account ) ).mapToDouble(
                Transfer::getAmount ).sum();
    ...
    public List<Entry<String, Integer>> findDeltas( final Integer year, final Integer month, final String account )
    {
        ...
        {
            double delta = 0d;
            delta -= transfersBySourceDate.findTransfers( date ).filter( transfer -> transfer.isOutgoing( account ) ).mapToDouble(
                Transfer::getAmount ).sum();
            delta += transfersByTargetDate.findTransfers( date ).filter( transfer -> transfer.isIncoming( account ) ).mapToDouble(
                Transfer::getAmount ).sum();
    ...
    {
        typeMap.get( "Mission" ).isOrChildOf( "Income" );

        return transfersByDate.findTransfers( year, month ).filter( transfer -> transfer.getType().isOrChildOf( type ) ).mapToDouble(
            transfer -> {
