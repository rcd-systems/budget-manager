...
import systems.rcd.bm.json.BmBudgetJsonInterfaceHandler;
...
public class Main
{
    ...
    private static RcdJettyServer launchServer()
        throws Exception
    {
        return RcdJettyService.createServer( "localhost", 0 ).addResourceHandler( "/res", Paths.get(
            "src/main/resources/systems/rcd/bm/res" ).toUri().toURL() )
            // .addResourceHandler("/res", Main.class.ggetResource("res"))
            .addHandler( "/json/transfers", new BmTransfersJsonInterfaceHandler() ).addHandler( "/json/budget",
                                                                                                new BmBudgetJsonInterfaceHandler() ).addHandler(
                "/json/sub-accounts", new BmSubAccountsJsonInterfaceHandler() ).addHandler( "/json/incoming-transfers",
                                                                                            new BmIncomingTransfersJsonInterfaceHandler() ).addHandler(
                "/json/outgoing-transfers", new BmOutgoingTransfersJsonInterfaceHandler() ).addHandler( "/json/years",
                                                                                                        new BmYearsJsonInterfaceHandler() ).addHandler(
                "/json/types", new BmTypesJsonInterfaceHandler() ).addHandler( "/json/accounts",
                                                                               new BmAccountsJsonInterfaceHandler() ).addHandler(
                "/json/deltas", new BmDeltasJsonInterfaceHandler() ).start();
    }

    private static void launchBrowser( final RcdJettyServer server )
        throws IOException
    {
