...
public class BmSubAccountsJsonInterfaceHandler implements RcdJettyHandler {
    ...
    public void handle(final String target, final HttpServletRequest request, final HttpServletResponse response)
        ...
        RcdContext.getService(BmModelService.class)
                .findAccounts()
                .stream()
                .filter(subAccount -> subAccount.isDirectChildOf(accountName))
                .map(subAccount -> toJsonObject(subAccount, year, month, endYear, endMonth))
                .forEach(jsonResponse::add);

        final Account account = RcdContext.getService(BmModelService.class)
