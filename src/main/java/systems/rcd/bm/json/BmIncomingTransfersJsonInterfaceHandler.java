...
public class BmIncomingTransfersJsonInterfaceHandler
    implements RcdJettyHandler
{
        ...
        throws IOException
    {
        ...
        final List<Transfer> transfers = RcdContext.getService( BmModelService.class ).findIncomingTransfers( year, month ).stream().filter(
            transfer -> transfer.isIncoming( account ) ).collect( Collectors.toList() );
